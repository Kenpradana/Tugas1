<x-layouts.app title="Tambah Obat">
    <div class="p-6 max-w-2xl mx-auto bg-white min-h-screen">
        <div class="card bg-base-100 shadow-md rounded-2xl border border-gray-100">
            <div class="card-body">
                <h2 class="card-title text-slate-800 mb-4">Tambah Obat Baru</h2>
                <form action="{{ route('obats.store') }}" method="POST">
                    @csrf
                    <div class="form-control w-full mb-4">
                        <label class="label font-semibold text-slate-700">Nama Obat</label>
                        <input type="text" name="nama_obat" placeholder="Contoh: Paracetamol" class="input input-bordered w-full" value="{{ old('nama_obat') }}" required>
                    </div>
                    <div class="form-control w-full mb-4">
                        <label class="label font-semibold text-slate-700">Kemasan</label>
                        <input type="text" name="kemasan" placeholder="Contoh: Tablet 500mg" class="input input-bordered w-full" value="{{ old('kemasan') }}" required>
                    </div>
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <div class="form-control w-full">
                            <label class="label font-semibold text-slate-700">Harga</label>
                            <input type="number" name="harga" placeholder="0" class="input input-bordered w-full" value="{{ old('harga') }}" required>
                        </div>
                        <div class="form-control w-full">
